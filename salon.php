...
<body>
                    ...
                    <div class="modal-body">
                        <div class="mb-3">
                            ...
                            <div class="input-icon">
                                <input type="text" id="buscarDocenteSalon" list="listaDocentes" class="form-control" placeholder="Escriba nombre o cédula..." required autocomplete="off">
                                <datalist id="listaDocentes">
                                    ...
                                    $r = mysqli_query($enlace, $q);
                                    while($doc = mysqli_fetch_array($r)) echo '<option value="'.$doc['nombre'].' '.$doc['apellido'].' (C.I: '.$doc['cedula'].')" data-id="'.$doc['id'].'">'; 
                                    ?>
                                </datalist>
                                <input type="hidden" name="docente" id="id_docente_hidden_salon">
    ...
    $(document).ready(function() {
        ...
        // Lógica para vincular el datalist con el input hidden de docente en salón
        const buscarDocenteInput = document.getElementById('buscarDocenteSalon');
        if (buscarDocenteInput) {
            buscarDocenteInput.addEventListener('input', function() {
                const val = this.value;
                const options = document.getElementById('listaDocentes').options;
                for (let i = 0; i < options.length; i++) {
                    if (options[i].value === val) {
                        document.getElementById('id_docente_hidden_salon').value = options[i].getAttribute('data-id');
                        break;
                    }
                }
            });
