...
class PDF extends FPDF
{
    ...
    function Header()
    {
        ...
        $this->Cell(35, 10, utf8_decode('Apellido'), 1, 0, 'C');
        $this->Cell(25, 10, utf8_decode('Cédula'), 1, 0, 'C');
        $this->Cell(35, 10, utf8_decode('Cargo'), 1, 0, 'C');
        $this->Cell(25, 10, utf8_decode('Fecha'), 1, 0, 'C');
        $this->Cell(25, 10, utf8_decode('Hora'), 1, 1, 'C');
    }
...
// Lógica de filtrado (igual que en asistencias.php)
...
$consulta = "SELECT * FROM asistencias $whereClause ORDER BY id DESC";
...
$pdf = new PDF();
$pdf->AliasNbPages();
$pdf->titulo = utf8_decode('Reporte de Asistencias');
$pdf->AddPage();
...
while ($fila = $resultado->fetch_assoc()) {
    ...
    $pdf->Cell(35, 10, utf8_decode($fila['apellido']), 1, 0, 'C', true);
    $pdf->Cell(25, 10, utf8_decode($fila['cedula']), 1, 0, 'C', true);
    $pdf->Cell(35, 10, utf8_decode($fila['cargo']), 1, 0, 'C', true);
    $pdf->Cell(25, 10, $fila['fecha'], 1, 0, 'C', true);
    $pdf->Cell(25, 10, $fila['hora'], 1, 1, 'C', true);
    $contador++;
}

$pdf->Output('D', 'reporte_asistencias.pdf');
exit();